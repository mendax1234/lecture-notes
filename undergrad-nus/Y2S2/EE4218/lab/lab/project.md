...
In the microprocessor design, our system starts from an **initial** PC address, which points to an instruction staying at that address of the IROM. During the running time, the PC value is controlled by the program that is being run itself. Another worth noting point is that, the instructions are stored in a **sequential manner** in the IROM. Thus, the program flow is not hard to be controlled by the program itself.
...
But, having only control signals is not enough, we still have to read our data, which is also the backbone of our **data packet**. To implement the data reading process, we draw the inspiration from the program counter to implement our **read counter**. Unlike the program counter which as mentioned above is controlled by the program that is being executed, the **read counter** is controled by the FSM.
