...
size_t power(size_t base, size_t time)
{
  size_t power = 1;
  for (size_t i = 0; i < time; i += 1)
  {
    power *= base;
  }
  return power;
...
int main()
{
  ...
  char **input = cs1010_read_word_array(n);
  if (input == NULL)
  {
    return 1;
  }
  size_t *result = calloc(n, sizeof(long));
  if (result == NULL)
  {
    return 1;
  }
  ...
  for (size_t i = 0; i < n; i += 1)
  {
    cs1010_println_size_t(result[i]);
  }
  for (size_t i = 0; i < n; i += 1)
  {
    free(input[i]);
