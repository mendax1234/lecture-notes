...
void print_res(char **res, size_t n)
{
  for (size_t i = 0; i < n; i += 1)
  {
    cs1010_println_string(res[i]);
  }
}
...
int main()
{
  ...
  char **res = calloc(num_test, sizeof(char *));
  for (size_t i = 0; i < num_test; i += 1)
  {
    size_t n = cs1010_read_size_t();
    size_t m = cs1010_read_size_t();
    char **str_arr = cs1010_read_word_array(n);
    res[i] = get_agree(str_arr, n, m); 
    for (size_t j = 0; j < n; j += 1)
    {
      free(str_arr[j]);
    }
