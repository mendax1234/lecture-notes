...
size_t find_max(person *people, size_t n)
{
  size_t max = 0;
  for (size_t i = 1; i < n; i += 1)
  {
    if (people[i].num > people[max].num)
    {
      max = i;
    }
  }
...
int main()
{
  ...
  {
    people[i].name = cs1010_read_word();
    people[i].num = cs1010_read_long();
  }
  size_t max = find_max(people, n);
  cs1010_println_string(people[max].name);
  for (size_t i = 0; i < n; i += 1)
  {
    free(people[i].name);
  }
