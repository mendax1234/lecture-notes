...
int main()
{
  ...
  student *class = calloc(n, sizeof(student));
  class[0].name = cs1010_read_word();
  for (size_t i = 1; i < n; i += 1)
  {
    class[i].name = cs1010_read_word();
    if (strcmp(class[i].name, PRESENT) == 0)
    {
      class[i-1].present = true;
    }
  }
  long count = 0;
  for (size_t i = 0; i < n; i += 1)
  {
    if (!class[i].present && strcmp(class[i].name, PRESENT) != 0)
    {
      cs1010_println_string(class[i].name);
      count += 1;
    }
  }
  if (count == 0)
  {
    cs1010_println_string("No Absences");
  }
  for (size_t i = 0; i < n; i += 1)
  {
    free(class[i].name);
