...
double find_avg(long *list, size_t num_stu)
{
  double sum = 0.0;
  for (size_t i = 0; i < num_stu; i += 1)
  {
    sum += (double)list[i];
  }
  return sum / (double)num_stu;
}

double percent_above_avg(long *list, size_t num_stu, double avg)
{
  long num = 0;
  for (size_t i = 0; i < num_stu; i += 1)
  {
  ...
  return (double)num / (double)num_stu * 100;
}

int main()
{
  size_t n = cs1010_read_size_t();
  double *above_avg = calloc(n, sizeof(double));
  if (above_avg == NULL)
  {
    return 1;
  }
  ...
  {
    size_t num_stu = cs1010_read_size_t();
    long *grade = calloc(num_stu, sizeof(long));
    if (grade == NULL)
    {
      return 1;
    }
    for (size_t j = 0; j < num_stu; j += 1)
    {
      grade[j] = cs1010_read_long();
    }
    double avg = find_avg(grade, num_stu);
    above_avg[i] = percent_above_avg(grade, num_stu, avg);
    free(grade);
  }
  for (size_t i = 0; i < n; i += 1)
  {
