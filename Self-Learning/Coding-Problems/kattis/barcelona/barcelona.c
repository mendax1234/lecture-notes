...
long find_pos(long *bags, long len, long tar)
{
...
int main()
{
  ...
  long tar = cs1010_read_long();
  long *bags = cs1010_read_long_array(n);
  long place = find_pos(bags, (long)n, tar);
  if (place == 0)
  {
    cs1010_println_string("fyrst");
  }
  else if (place == 1)
  {
    cs1010_println_string("naestfyrst");
  }
  else
  {
    cs1010_print_long(place + 1);
    cs1010_println_string(" fyrst");
  }
}
