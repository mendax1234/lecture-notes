...
void update_within_bound_vertical(long *y, long dist, long length)
{
  *y += dist;
  if (*y >= length)
  {
...
void update_within_bound_horizontal(long *x, long dist, long width)
{
  *x += dist;
  if (*x >= width)
  {
...
void calc_loc(long seg, long width, long length, long *act_x, long *act_y, long *vir_x, long *vir_y)
{
  for (long i = 0; i < seg; i += 1)
  {
    char dir = getchar();
    long dist = cs1010_read_long();
    if (dir == 'd' || dir == 'l')
    {
      dist *= -1;
    }
    ...
    {
      *vir_y += dist;
      update_within_bound_vertical(act_y, dist, length);
    }
    else if (dir == 'l' || dir == 'r')
    {
      *vir_x += dist;
      update_within_bound_horizontal(act_x, dist, width);
    }
  }
}
...
int main()
{
  ...
  while (1)
  {
    width = cs1010_read_long();
    length = cs1010_read_long();
    if (width == 0 && length == 0)
    {
      return 0;
    }
    long seg = cs1010_read_long();
    // Calculate the virtual and actual location
    long virt_x = 0;
    long virt_y = 0;
    long actu_x = 0;
    long actu_y = 0;
    calc_loc(seg, width, length, &actu_x, &actu_y, &virt_x, &virt_y);
    // Print res
    printf("Robot thinks %ld %ld\n", virt_x, virt_y);
    printf("Actually at %ld %ld\n", actu_x, actu_y);
  } 
}
