...
metaLinks:
  alternates:
    - /broken/spaces/W45nwClYZdzz9MQG1dUb/pages/yC1muqnntbZFhP15VFCW
---
...
The **bridge** is responsible for converting AXI transactions into a streaming data format suitable for high-throughput hardware processing. So the exact steps from PS to the bridge are as follows:

1. Laptop sends data via **UART**.
2. The processor uses a `lw` instruction to read the data from the UART register. The data read will be stored in a register first.
3. The processor uses a `sw` instruction to writes to the **Bridge** module's **status register** to wake it up and its **data register** to give the Bridge module the data to be sent to the coprocessor.
4. The **bridge module** converts the data received into **AXI Stream** and send it to the coprocessor to process the data.
...
**AXI** is part of ARM AMBA, a family of microcontroller buses first introduced in 1996. The first version of AXI was first included in AMBA 3.0, released in 2003. AMBA 4.0, released in 2010, includes the second version of AXI, **AXI4**. There are 3 types of AXI4 interfaces:
...
* `TLAST` is an indication from the **master** to slave that the current data word is the last.

{% hint style="danger" %}
`TLAST` is considered a sideband signal and is optional for AXIS. All the other signals mentioned above are essential signals for AXIS. It is useful in scenarios where the slave doesn't know exactly how many data words are sent by the master and is required if the slave is AXI Stream FIFO or AXI DMA, as these IPs (we will see in Lab 3) expect it.
{% endhint %}
...
1. 2 RAMs: One to store matrix A and the other to store matrix B.
...
I use the FSMD (FSM with Datapath) which has occured in the [Mach-V mul & div unit](https://mendax1234.github.io/Mach-V/hw/uarch/mul-div-unit/) design. The core idea for the FSMD design is that
...
2. **FSM -> System Outputs**
   1. `S_AXIS_TREADY`: Signals to the master[^4] that the coprocessor is ready to accept input.
   2. `M_AXIS_TVALID`: Signals to the slave[^5] that the output data from the coprocessor is valid.
   3. `M_AXIS_TLAST`: Signals that the current output data from the coprocessor is the last word in the packet.
...
The fundamental rule for using AXIS to transfer data is that
...
5. **Termination**: On the final word, the Testbench sets `S_AXIS_TLAST = 1`. After the loop finishes, it deasserts `S_AXIS_TVALID = 0`.
...
2. At 1350ns, the `S_AXIS_TVALID` is kept HIGH for another 100ns for the last data to be written into `B_RAM`. In the meantime, the `START` of the matrix multiply unit is asserted HIGH.
   1. After optimization, at 1250ns, the `START` is triggered High and remains one cycle, this can save one cycle for the `Compute` stage.
3. At 1425ns, immediately after `S_AXIS_TVALID` goes down, `M_AXIS_TREADY` goes up, meaning that the testbench is ready to receive the data from the coprocessor! But let's wait for the `Compute` stage to complete first.
4. The matrix multiply unit starts computing.

{% hint style="warning" %}
The write process to RAM is done when
...
In Lab 01, we also use the FSM to implement our matrix multiply unit. The meanings of some important variables are shown as follows:

| Variable | Logical Meaning   | Range / Size     | Pipeline Action                                                                |
| -------- | ----------------- | ---------------- | ------------------------------------------------------------------------------ |
...
{% hint style="warning" %}
We are doing 2x4 times 4x1 matrix multiplication in this lab.
{% endhint %}

In the multiply and accumulate stage, we have three pipelines

| Stage    | Primary Task       | Key Logic / Operation                                                           | Pipeline Flow                                                                                   |
| -------- | ------------------ | ------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
...
[^4]: The processor system.

[^5]: The processor system, same as the master mentioned above.
