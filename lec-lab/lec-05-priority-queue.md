...
$$
\begin{align*}
\sum_{h=0}^{\lfloor \lg n \rfloor} \left \lceil\frac{n}{2^{h+1}}\right \rceil \cdot O(h)  &= \sum_{h=0}^{\lfloor \lg n \rfloor} \left\lceil \frac{n}{2^{h+1}}\right\rceil \cdot (c\cdot h) \\
